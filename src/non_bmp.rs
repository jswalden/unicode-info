...
pub fn generate_non_bmp_info(code_point_table: &code_point_table::CodePointTable) -> NonBMPInfo {
  let DerivedCorePropertyData { id_start: derived_id_start, id_continue: derived_id_continue } =
...
fn non_bmp_space_set_is_empty() {
  ...
  assert!(non_bmp_info.space_set.is_empty(),
...
fn non_bmp_identifier_start() {
  ...
  assert_eq!(table.name(OLD_PERSIAN_SIGN_AURAMAZDAA),
             "OLD PERSIAN SIGN AURAMAZDAA");

  let non_bmp_info = generate_non_bmp_info(&table);

  assert!(non_bmp_info.id_start_set.contains(&OLD_PERSIAN_SIGN_AURAMAZDAA),
